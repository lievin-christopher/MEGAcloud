...
static int MEGAopen(const char *p, struct fuse_file_info *fi)
{
    return 0;
}
...
static int MEGAcreate(const char *p, mode_t, struct fuse_file_info *)
{
	ofstream outfile;
	string tmpfilename = p;
	tmpfilename = tmpfilename.substr(tmpfilename.find_last_of("/\\") + 1);
	...
	if (n)
	{
		MegaApi::log(MegaApi::LOG_LEVEL_WARNING, "File already exists");
		delete n;
		return -EEXIST;
	}
	string spath = path;
	...
	SynchronousTransferListenerFuse listener;
	megaApi->startUpload(tmpfilename.c_str(), n, &listener);
	listener.wait();
	return 0;
}

static int MEGAchmod (const char *p, mode_t)
{
	return 0;
}

static int MEGAchown (const char *, uid_t, gid_t)
{
	return 0;
}

static int MEGAtruncate (const char *, off_t)
{
	return 0;
}

static int MEGAwrite(const char *p, const char *buf, size_t buf_siz, off_t buf_offset, struct fuse_file_info *fi)
{
	ifstream infile;
	string tmpfilename = p;
	tmpfilename = tmpfilename.substr(tmpfilename.find_last_of("/\\") + 1);
	...
	if (infile) {
		...
		ofstream outfile;
		outfile.open(tmpfilename,ios::binary);
		outfile.seekp(buf_offset);
		outfile.write(buf,buf_siz);
		outfile.close();
	} else {
		string path = megaBasePath + p;
		size_t size;
		MegaApi::log(MegaApi::LOG_LEVEL_DEBUG, "Caching file:");
		MegaApi::log(MegaApi::LOG_LEVEL_DEBUG, path.c_str());
		MegaNode *node = megaApi->getNodeByPath(path.c_str());
		if (!node)
		{
		...
		size = node->getSize();		
		SynchronousTransferListenerFuse listener;
		megaApi->startStreaming(node, 0, size, &listener);
		listener.wait();
		delete node;
		if (listener.getError()->getErrorCode() != MegaError::API_OK)
		{
		...
		outfile.open(tmpfilename,ios::binary);
		outfile << listener.getData();
		MegaApi::log(MegaApi::LOG_LEVEL_DEBUG, "File cache OK");
		outfile.seekp(buf_offset);
		outfile.write(buf,buf_siz);
		outfile.close();
	}
...
int main(int argc, char *argv[])
{
	...
	ops.release		= MEGArelease;
	ops.create		= MEGAcreate;
	ops.write 		= MEGAwrite;
	ops.chmod 		= MEGAchmod;
	ops.chown 		= MEGAchown;
	ops.truncate 	= MEGAtruncate;
	
	
    
	char *fuseargv[5] = { argv[0], (char *)"-f", (char *)"-d", (char *)"-s", (char *)mountpoint.c_str()};
    return fuse_main(5, fuseargv, &ops, NULL);
}
